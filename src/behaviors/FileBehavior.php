...
class FileBehavior extends Behavior
{
    ...
    /**
     * @return File query object to be used by other methods
     */
    ...
    /**
     * @return File[] prepared for blueimp gallery
     * @throws \Exception
     */
    public function getGalleryFiles()
    {
        $files = $this->getFiles();
            $items = [];
            foreach ($files as $file){
                if(!in_array($this->mimesInGallery,$file->mime))
                    throw new InvalidConfigException("The behavior {FileBehavior} has not been attached to the model.");
                $items[] = [
                    'url' => $file->url,
                    'src' => $file->name, //thumbnail path
                    'options' => array('title' => $file->name.$file->type,'type' => $file->mime)
                ];
            }
            return $items;
    }
    ...
    public function getFilesByType($type)
    {
        ...
        return $fileQuery->andFilterWhere(['type'=>$type]);
    }
    /**
     * @return File[] images only
     * @throws \Exception
     */
    public function getImageFiles($options=false)
    {

        $fileQuery = $this->fileQuery();
        $imageFiles = $fileQuery->andFilterWhere(['like', 'mime', 'image'])->all();
        if(is_array($options) && $options['preview']){
            return $this->getImageGalleryFiles();
        }
        return $imageFiles;
    }
 ...
 */
    ...
    public function getTextGalleryFiles()
    {
        $textFiles = $this->getTextFiles();
        $items = [];
        foreach ($textFiles as $file){
            $items[] = [
                'url' => $file->url,
                'src' => $file->name, //thumbnail path
                'options' => array('title' => $file->name.$file->type,'type' => 'text/html')
            ];
        }
        return $items;
    }
